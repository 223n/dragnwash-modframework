...
- **The keys move through the list** in the left pane, in both views (the tree and the search results in Scene, the folders and objects in Objects), while no field has the keyboard: up and down one row, Page up and Page down a pane, Home and End the ends. Right opens a folder (or a node with children) and, when it is open already, goes to the first row in it; left closes it, and on a row that has nothing to close goes up to the row it is in. Moving onto an object selects it, so the right side follows the keys; in a narrow window the list stays in view (a click still turns the page). The list scrolls only as far as it must to keep the row in view, so stepping does not move it about. In Objects the keys also stop on folder rows, which are no selection: the folder they are on is marked like a selected row. On the Steam Deck, and with any gamepad, the **d-pad** is those keys: the window sends it on as real arrow keys, so it steps a row, opens and closes, and repeats while it is held, and a list is walked with no keyboard at all. The sticks are left to scroll.
