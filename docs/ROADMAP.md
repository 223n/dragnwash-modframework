...
- **Operations, MCP and node graphs.** Each library registers what it can do as named operations; from that one registry come Console commands, MCP tools for AI clients (read-only first, this computer only, off by default) and blocks for node graphs built outside the game and shipped as data-only mods. In four stages: the registry, MCP, code seen as nodes, building ([plan](API_PLAN.md)); building is designed ([graphs](GRAPHS.md)).
