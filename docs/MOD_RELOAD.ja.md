...
> **実験的な機能です。** 中核（1.2.0）に作り、`main` にありますが、まだリリースには含まれていません。設計と違う点：`UnloadOwned` は `ModReload.Unloading` というイベントになり、各ライブラリが自分の登録を外します（GUID を持つものは GUID で、そうでないものは古いビルドのアセンブリで `ModReload.Prune`）。Options の行は外さず、新しいビルドがもう一度足したときに付け替えます。Mods 画面はまだリロードしたことを表示しません（Console の `mods` は表示します）。
