...
> **実験的な機能です。** 中核（1.2.0）に作り、`main` にありますが、まだリリースには含まれていません。設計と違う点：`UnloadOwned` は `ModReload.Unloading` というイベントになり、各ライブラリが自分の登録を外します（GUID を持つものは GUID で、そうでないものは古いビルドのアセンブリで `ModReload.Prune`）。Options の行は外さず、新しいビルドがもう一度足したときに付け替えます。Mods 画面はまだリロードしたことを表示しません（Console の `mods` は表示します）。ビルドは `<Mod>.dll.new` として届けます。Windows では動いている DLL がロックされていて上書きできないためで、次の起動時にプリローダーが `.new` を本物の DLL にします。
...
1. **監視。** 開発者ツールがオンで、ゲームの窓が前面にある間（バックグラウンドでは Unity の Update が止まるので、その間に届いたビルドは窓が戻ったときに読み込まれます）、対応を宣言した Mod（後述）について `BepInEx/plugins/<Mod>/*.dll` を見張ります。ファイルが変わってから 0.5 秒静かになったら発火します。ビルドのコピーが終わるのを待つためです。
