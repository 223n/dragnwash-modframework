...
- **操作の登録簿、MCP、ノードグラフ。** 各ライブラリが、できることを名前つきの操作として登録します。その 1 つの登録簿から、Console コマンド、AI クライアント向けの MCP のツール（まず読むだけ、この PC の中だけ、既定はオフ）、ゲームの外で組み立てて DLL のない Mod として配れるノードグラフのブロックを作ります。4 段階で進めます：登録簿、MCP、コードをノードで眺める、組み立て（[計画](API_PLAN.ja.md)）。組み立ては設計ができています（[グラフ](GRAPHS.ja.md)）。
