...
- **Graphs 0.1.0：何かをする、コードのない Mod**（[計画](API_PLAN.ja.md)の第 4 段階、[設計](GRAPHS.ja.md)）。`mod.json` と `graphs/*.json` を置いたフォルダーが、各ライブラリの上げるイベントに応えて、登録された操作を呼びます。ファイルは動き出す前に全部検査し、すべてのグラフで 1 フレームあたり 1 ミリ秒を分け合い、3 回続けて失敗したグラフは変更を戻して止めます。`main` にあり、ゲームでも動かしましたが、最初の書き込み操作とエディターが揃うまではリリースに入れません。それが無いと、グラフは読むこととログを書くことしかできないからです。
