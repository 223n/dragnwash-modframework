...
> **Experimental.** Built in the core (1.2.0), on `main` but not yet in a release. Where the build differs from the design: `UnloadOwned` became `ModReload.Unloading`, an event each library handles for its own registrations (by GUID where it kept one, else by the old build's assembly with `ModReload.Prune`); an Options row is not removed but re-pointed when the new build adds it again; the Mods screen says how many times a mod was reloaded this session, as does `mods` in the Console; and a build is delivered as `<Mod>.dll.new`, because on Windows the running DLL is locked and cannot be overwritten (the preloader makes the `.new` the real DLL at the next launch).
