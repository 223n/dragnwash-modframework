...
一番下の行に打って Enter。打っている間、次に来られる語が行の上に並びます。最初の語ならコマンド名、そのあとはコマンドが用意した候補（`log` ならレベルや出どころ、`assets` なら `textures` や `reload` など）です。**Tab** で選ばれている候補を入力し、↑↓で候補を移動、Esc で閉じます。候補がないときは↑↓で履歴をたどれます。最初から入っているもの：
...
- 5 つ目の引数で補完を用意できます。名前のあとに打たれた語（最後は途中の語か、空白直後なら ""）を受け取り、そこに来られる語を返します。
