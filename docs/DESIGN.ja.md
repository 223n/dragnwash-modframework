...
- **競合を見えるようにする。** Mods 画面に、検出できる問題を表示します。足りない・古すぎるライブラリ、`BepInIncompatibility`、読み込まれなかったプラグインとその理由、そして（予定）複数の Mod が直接パッチを当てているゲームのメソッドです
- **黙って置き換えない。** サービスの提供者は 1 つだけです。2 つ目の登録は拒否してログに出します
- **作者向けのガイドライン。** ドキュメントに、フレームワークの何を使うか、何に直接パッチを当てないか、ほかの人が使えるライブラリの作り方を書きます
...
4. **0.4 拡張ポイント。** サービスの登録、`ModInfo.IsLibrary` と Mods 画面でのライブラリ表示、Mods 画面へのページ追加、動作チェック
5. **ライブラリ**を 1 つずつ、翻訳 Mod が必要とする順に：テキスト、会話、ツールウィンドウ、アセット、フラグとセーブ。それぞれが独自のバージョンを持つ別のプラグイン
6. 翻訳 Mod v1.0.0 が中核と使うライブラリの上で動いたら、**中核の 1.0**
