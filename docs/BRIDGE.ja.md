...
1. **Host**：`Host` ヘッダーは `127.0.0.1:<ポート>` か `localhost:<ポート>` でなければなりません。Bridge が待ち受けているポートと比べます。自分の名前を 127.0.0.1 に向けるウェブサイト（DNS リバインディング）は、自分の名前を送ってくるので断れます。ページ専用の入り口（docs/CODE_GRAPH.md）も、`Host` と、`Origin` をこの 2 つのアドレスと、自分でもう一度確かめます。
