...
> [!NOTE]
> **Core 1.2.0** is out as a **pre-release** while it is checked; the latest full release is 1.1.2 with the libraries at 1.0.0. 1.2.0 adds the developer-tools switch (off by default), text and key-binding fields on the settings pages, [`GameEvents`](docs/GUIDE.md), `SettingMeta`, reloading a mod while the game runs, and [mods that go online say so](docs/NETWORK.md); with it the Tool window, Assets and Dialogue libraries go to 1.1.0 ([Console](docs/CONSOLE.md), [texture replacements and reloading](docs/ASSET_TOOL.md), [stable line keys](docs/STABLE_LINE_KEYS.md)) and a new [Inspector](docs/INSPECTOR.md) library (1.0.0) arrives. The new features are marked experimental; the Inspector stays experimental after the full release too. 1.1.0 added [update notices](#update-notices), the shared installer and uninstalling from the Mods screen; 1.1.1 added the framework's icon, and 1.1.2 replaces it with the "Dg" monogram from the hand-made logo. 1.0.0 was released together with [Drag'n Wash Localization](https://github.com/TomXV/dragnwash-localization) v1.0.0, the first mod built on it. From 1.0.0 on, a change that breaks the public API comes only with a new major version; see [CHANGELOG.md](CHANGELOG.md).
>
