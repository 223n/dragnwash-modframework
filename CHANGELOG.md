...
- Experimental. Animators, read by reflection (no animation module is referenced): a selected Animator's parameters (Float, Int, Bool, Trigger) and layer weights are rows among its members, edited and kept in History like any member; one line per layer shows its weight, the clips it plays, how far through and the blend to the next state; **Pause animation** (speed 0, put back on Resume, when the window closes or when developer tools go off) with **Step** (1/30 s) and a time slider per layer. See docs/INSPECTOR.md, Animators.
