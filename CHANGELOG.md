...
- `mod-install.json` schema 2 adds a `framework` block: the ModFramework release the mod pins (`version`), the SHA-256 and size of its zip, and `needs`, the lowest version of each plugin folder the mod uses (`DragNWash.ModFramework`, `DragNWash.ModFramework.Text`, ...). Every field is checked: a version of digits and dots, 64 lowercase hex digits, a size above 0 and at most 20 MB, folder names that start with `DragNWash.ModFramework`. Schema 1 (the framework inside the mod's zip) installs as before; a zip that brings the framework is used even with schema 2. A schema newer than 2 is refused with "Use the installer from the mod's release". `installer/mod-install.example.json` is schema 2.
- Install.exe: when the mod's zip has no framework, it installs the core, the preloader and only the libraries in `needs` from the pinned release (never Inspector, Overrides, Bridge or Graphs). A part that is missing, below the mod's minimum, or (the core and the preloader) older than the pinned release is fetched; each part is then updated only when the release's is newer, so a newer one another mod brought is kept. Within a framework folder only the files in the zip are written. When everything is already there, nothing is downloaded and no connection is made.
- The zip comes from `https://github.com/TomXV/dragnwash-modframework/releases/download/v<version>/DragNWash.ModFramework-<version>.zip`, an address the installer builds from the version alone (never from the manifest; `api.github.com` is never asked), into a temp folder of the run's own. It stops as soon as the size differs from the manifest's (or passes 20 MB), is used only when its SHA-256 matches, and only the needed folders are unpacked into the staging folder, then copied into place with the same backup and putting back as the rest of the install. Both downloads send the User-Agent `DragNWash.Installer/1.1.0`, wait at most 30 seconds, and try once more after a server error or time-out.
- A window before the first connection, only when a download is needed and every time: where it connects (github.com and release-assets.githubusercontent.com; BepInEx/BepInEx too when BepInEx is downloaded), why, what is sent (an ordinary HTTPS request and the User-Agent; GitHub sees the IP address), how the file is checked, a link to the release page, and **Choose zip...**, **Download and install** or **Cancel**. A zip chosen there is checked the same way, with no internet.
- The list of what Install will do names both hosts. While Install runs it becomes a checklist (done, running, still to do) with the download's KB; afterwards it says what was updated, what was kept and why, which libraries were not installed, that other mods' folders were not touched, and where the backup is. The status line shows the installed ModFramework version, and the SmartScreen line says the installer downloads ModFramework.
- A failed download says why: no internet, GitHub not answering, GitHub limiting downloads (with the minutes it gives), the release not published yet (tell the mod's author), a size or SHA-256 that does not match (the file is deleted), or a secure connection that could not be confirmed (no way around it). The failure window links the release page, has **Choose zip...**, and, when the installed framework meets every minimum, "Install only the mod and keep ModFramework X". The game folder is unchanged in every case.
- Command line: `--framework-zip <file>` (a local zip, checked the same way), `--no-download` (never online; stops, with nothing changed, when BepInEx or ModFramework would have to be downloaded), `--keep-framework` (the failure window's "install only the mod"), and `--help`. `--install` counts as consent, as it does for BepInEx.
- Steam Deck script (`install-steamdeck.sh`): the same flow in the terminal or kdialog - it asks first (where, why, what is sent), fetches the pinned release with the same address and User-Agent, checks size and SHA-256, installs only what `needs` names, never an older part over a newer one, and puts the game folder back when a copy fails part way. `--framework-zip` and `--no-download` work as in Install.exe. A choice prompt in the terminal now reads what is typed instead of always taking the default.
- Build workflow: each release carries `SHA256SUMS` (`<sha256>  DragNWash.ModFramework-<version>.zip`) next to the zip, in the draft release and the workflow artifact.
...
- The core writes two `[startup]` blocks to the log once per launch, timed to the millisecond: when BepInEx has loaded the plugins (process start to the core, the core to the end of loading, each plugin's load time, and the longest gaps between log lines with the line that ended each), and 10 seconds after the first scene (how long it took to come, frames slower than 100 ms with the lines written in them, and the longest gaps). Then it stops listening, so it costs nothing afterwards. It runs only when developer tools are on at launch (Options → Mods → Drag'n Wash ModFramework → Developer tools), as it costs a few tens of milliseconds; with them off, it lets go of the log in the core's Awake and none of it runs.
- The block after the first scene also breaks the first frame after it down: each plugin's Start (a coroutine's up to its first yield), Update, LateUpdate and OnGUI, and each `sceneLoaded` handler a mod added, in milliseconds, then the timing's own share and the rest (the game's own and Unity's). They are timed by Harmony patches put on when that scene has loaded and taken off after the frame; what putting them on and taking them off cost is in the block too.
- Connection watching looks for `HttpClient` in System.Net.Http only, so the core's start no longer spends about 100 ms, and three HarmonyX "Could not find type" warnings, on it when the game has not loaded that assembly. `HttpClient` is watched as before once it is loaded.
