...
> [!NOTE]
> **中核 1.2.0** を、確認のあいだ**プレリリース**として公開しています。最新の正式リリースは 1.1.2（ライブラリは 1.0.0）です。1.2.0 では開発者ツールのスイッチ（既定はオフ）、設定ページの文字列とキー割り当ての入力欄、[`GameEvents`](docs/GUIDE.ja.md)、`SettingMeta`、ゲームを動かしたままの Mod のリロード、[外と通信する Mod の申告](docs/NETWORK.ja.md) が加わり、あわせて Tool window、Assets、Dialogue の各ライブラリが 1.1.0 になり、新しい [Inspector](docs/INSPECTOR.ja.md) ライブラリ（1.0.0）が加わります（[Console](docs/CONSOLE.ja.md)、[テクスチャの差し替えとリロード](docs/ASSET_TOOL.ja.md)、[安定した行キー](docs/STABLE_LINE_KEYS.ja.md)）。新しい機能は実験的な扱いで、Inspector は正式リリースのあとも実験的な機能のままです。1.1.0 で [更新のお知らせ](#更新のお知らせ)、共通インストーラー、Mods 画面からのアンインストールを追加し、1.1.1 でフレームワークのアイコンを追加、1.1.2 でそのアイコンを手作りのロゴの「Dg」に差し替えました。1.0.0 は、最初にこの上で動く Mod である [Drag'n Wash Localization](https://github.com/TomXV/dragnwash-localization) の v1.0.0 と一緒にリリースしました。1.0.0 以降、公開 API の互換性を壊す変更はメジャーバージョンを上げるときだけにします。[CHANGELOG.md](CHANGELOG.md) を参照してください。
>
